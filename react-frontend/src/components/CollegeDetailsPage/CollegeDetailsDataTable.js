...
const CollegeDetailsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.HOD}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "collegeDetails"});
...
<Column field="HOD" header="HOD" body={pTemplate1} filter={selectedFilterFields.includes("HOD")} hidden={selectedHideFields?.includes("HOD")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload CollegeDetails Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search CollegeDetails" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CollegeDetailsDataTable;