...
import collegeDetailsFakerFactory from "./collegeDetailsFakerFactory";
...
const CollegeDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = collegeDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("collegeDetails").create(elem));
...
export default connect(mapState, mapDispatch)(CollegeDetailsFakerDialogComponent);
